...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-[260px] flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto ${
      ...
      >
        ...
        <div className="mx-4 mt-4 p-3 rounded-xl bg-white/8 border border-white/10">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-accent-400 to-accent-600 flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-md">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          <p className="px-3 mb-2 text-[11px] font-bold text-white/30 uppercase tracking-wider">Menu</p>
          ...
          {role === UserRole.ADMIN && (
            <>
              <p className="px-3 mt-5 mb-2 text-[11px] font-bold text-white/30 uppercase tracking-wider">Administração</p>
          ...
          {role === UserRole.COMPRADOR && (
            <>
              <p className="px-3 mt-5 mb-2 text-[11px] font-bold text-white/30 uppercase tracking-wider">Gerenciamento</p>
              <NavLink to={ROUTES.USERS} className={navItemClass} onClick={onClose}>
                <Users className="h-5 w-5 shrink-0" />
                Vendedores
              </NavLink>
        ...
        <div className="border-t border-white/10 px-3 py-3 space-y-1">
          ...
          <button
            onClick={signOut}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-red-300 hover:bg-red-500/15 transition-all duration-200 w-full"
          >
